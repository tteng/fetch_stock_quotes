    ...
    return request.get(sprintf(options.uri, query), function(error, res, body) {
      if (body !== void 0) {
        body.replace(/"/g, '').split('\n').forEach(function(line) {
          var ary, i, sql, stock_params, _i, _ref;
          line = line.trim();
          if (line === '') {
            return;
          }
          ...
          return mysql.query(sql, null, function(err, results) {
            if (err) {
              console.log(err);
              response.write("\n");
              ...
              memcached.set("" + env.MEMCACHED_NAMESPACE + ":" + ary[0], ary[3], env.STOCK_PRICE_EXPIRES, function(error, result) {
                if (error) {
                  return console.log("[cache:error:write] set price failed " + ary[0] + "-" + ary[3] + " caused by " + error);
                }
              });
            }
            console.log(results);
