...
fetch_tags_cn21_score = (response, query) ->
              ...
              mysql.query get_tag_sql, null, (err, tag_results) ->
                if err
                  ...
                  mysql.query get_proxy_sql, null, (err, results) -> 
                    if err 
                      console.error "[mysql_error:fetch_tags_cn21_score] can't find available proxy by #{err}"
                    else
                      ...
                      request.get(
                        {
                          ...
                          timeout: 30000000
                             ...
                             update_all_tags_cn21_score() if completed_count >= tags_total_count
                      )
                  step_idx++
...
update_all_tags_cn21_score = ->
  ...
  mysql.query update_tags_score_sql, null, (err, results) ->
    if err
      console.log "[mysql error] update tags cn21 score failed for #{err}"
    else
      console.log "tags cn21 score updated"

single_tag_cn21_rank = (response, query) ->
  query_option = {}
  ...
  request.post(
    { 
      ...
      timeout: 30000000, #默认超时时间设置为30秒 
      headers: {"User-Agent": "Safari 10.2"},
      form: {"query": decodeURI(tag)}
    },(error, res, body) ->
        if error 
