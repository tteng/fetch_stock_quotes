...
options     = require('./google_request_properties').query_options
jsdom       = require 'jsdom'
...
fetch_tags_google_score = (response, query) ->
              ...
              mysql.query get_tag_sql, null, (err, tag_results) ->
                if err
                  ...
                  mysql.query get_proxy_sql, null, (err, results) -> 
                    if err 
                      console.error "[mysql_error:fetch_tags_google_score] can't find available proxy by #{err}"
                    else
                      ...
                      request.get(
                        {
                          ...
                          timeout: 30000000
                             ...
                             update_all_tags_google_score() if completed_count >= tags_total_count
                      )
                  step_idx++
...
update_all_tags_google_score = ->
  console.log "[tags:google:ranks] all tags' google rank updated" 
  update_tags_score_sql = "call prcd_update_tags_google_score();"
  mysql.query update_tags_score_sql, null, (err, results) ->
    if err
      console.log "[mysql error] update tags google score failed for #{err}"
    else
      console.log "tags google score updated"

single_tag_google_rank = (response, query) ->
  query_option = {}
  ...
  logger.info "--- proxy: #{query_option.proxy}:#{query_option.port}"

  update_sql = "update tags set google_rank=?, updated_at=now() where name=?"
        ...
        if error 
          ...
          jsdom.env {html: body}, (error, window) ->
                                    if error 
                                      ...
                                      reg_pat =  /About\s+((\d{1,},?)+\s) results/im
                                      ...
                                      unless match == null 
                                        ...
                                        mysql.query update_sql, [score, decodeURI(tag)], (err, results) ->
                                          if err
                                            consloe.log "[mysql error] update google rank failed for #{err}"
                                          window.close()
                                          response.end()
                                      else
...
calc_top_hundred_score = (response, query) ->
  update_all_tags_google_score()
  response.write("all tags google score will be updated")
  response.end()

exports.single_tag_google_rank = single_tag_google_rank
exports.fetch_tags_google_score = fetch_tags_google_score
exports.calc_top_hundred_google_score = calc_top_hundred_score
